...
**Corrección 2026-07-12 (misma versión 01.18): la nota de arriba quedó desactualizada por el propio trabajo que describe — el audio de cutscenes SÍ se reparó, y Círculo NO quedó en el keycode 97.** Detalle completo en Fixes_Log.md §17, resumen aquí para no dejar esta nota contradiciendo el código real:

- **Audio de cutscenes:** la nota de arriba decía que "debe permanecer forzado a 1024 frames por seguridad" porque variar el tamaño dinámicamente rompía `sceAudioOutOpenPort`. Eso describía un síntoma real pero mal diagnosticado — la falla real de apertura (`0x80260008` = `SCE_AUDIO_OUT_ERROR_INVALID_SAMPLE_FREQ`, confirmado leyendo el propio header de vitasdk) era por el TIPO de puerto (`SCE_AUDIO_OUT_PORT_TYPE_MAIN` exige 48000Hz exactos, y casi todos los videos son 44100Hz), no por el tamaño del paquete. Con el puerto abierto como `SCE_AUDIO_OUT_PORT_TYPE_VOICE` (sin esa restricción), sí se pudo variar el tamaño dinámicamente — de hecho HAY que hacerlo, derivándolo de `audio.details.audio.size` en cada cutscene, porque `sceAudioOutOutput()` no recibe un largo por llamada: siempre reproduce exactamente el `len` fijado al abrir el puerto, y forzarlo a 1024 cuando el frame real trae otro tamaño es precisamente lo que causaba el audio entrecortado. Confirmado por el usuario en consola (log_000039: "audio entrecortado... pero al menos suena" ya con el puerto en VOICE; el fix del tamaño dinámico se aplicó inmediatamente después).
- **Círculo:** terminó en el keycode 20 (`DPAD_DOWN`), NO en 97 (`BUTTON_B`) — el 97 fue descartado en la MISMA sesión al confirmarse en consola que el motor nunca reacciona a él ("no hace nada en gameplay"). Como Abajo/joystick-abajo ya usan el 20 para agacharse, ambos ahora comparten ese keycode con la transición de presión/soltado calculada sobre su OR combinado (para que soltar uno mientras el otro sigue presionado no cancele el agachado).
- **Bug no documentado en la nota original:** el video, al dibujar su primer frame real (posible recién con el fix de memoria de esta misma versión), dejaba la proyección de GL y el estado de blend/depth-test corrompidos para el resto del juego — pantalla invertida tipo espejo y sprites con cuadros negros justo después de cualquier cinemática. Ver Fixes_Log.md §17 para el detalle completo.

**Lección:** cuando una nota de "issue conocido" se escribe con una hipótesis (el hardcode de 1024 "por seguridad") en vez de con el código de error real devuelto por la propia API, vale la pena releer el header del SDK antes de aceptar la hipótesis como definitiva — el propio `audioout.h` documentaba la restricción de 48000Hz de `MAIN` todo este tiempo.
