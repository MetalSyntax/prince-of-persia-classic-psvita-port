/* main.c -- Prince of Persia Classic (Cocos2d-x) .so loader
 *
 * Ported based on original loader code.
 */
...
// Updated data path for POP Classic
...
// --- PTHREAD WRAPPERS ---
// (Kept largely the same as they are standard wrapper implementations)
...
// File system hooks adapted for ux0:data/popclassic
...
int lstat_hook(const char *pathname, stat64_bionic *statbuf) {
	...
	if (strncmp(pathname, "ux0:", 4)) {
		char fname[256];
		sprintf(fname, "%s/%s", data_path, pathname);
		dlog("lstat(%s) fixed\n", fname);
		res = stat(fname, &st);
	} else {
...
void *dlsym_hook( void *handle, const char *symbol) {
	dlog("dlsym %s\n", symbol);
    // Try resolving in specific modules if standard logic fails or as fallback
...
int access_hook(const char *pathname, int mode) {
	...
	if (strncmp(pathname, "ux0:", 4)) {
		char real_fname[256];
		sprintf(real_fname, "%s/%s", data_path, pathname);
...
FILE *AAssetManager_open(void *mgr, const char *fname, int mode) {
	char full_fname[256];
	sprintf(full_fname, "%s/%s", data_path, fname);
...
int rename_hook(const char *old_filename, const char *new_filename) {
	...
	if (strncmp(old_filename, "ux0:", 4)) {
		sprintf(real_old, "%s/%s", data_path, old_filename);
	} else {
		strcpy(real_old, old_filename);
	}
	if (strncmp(new_filename, "ux0:", 4)) {
		sprintf(real_new, "%s/%s", data_path, new_filename);
	} else {
...
void *pthread_main(void *arg) {
    // Updated for POP Classic standard Cocos2dx JNI calls
	int (* JNI_OnLoad) (void *vm, void *reserved) = (void *)so_symbol(&game_mod, "JNI_OnLoad");
	void (* Engine_Init) (void *env, void *obj, int w, int h) = (void *)so_symbol(&game_mod, "Java_org_cocos2dx_lib_Cocos2dxRenderer_nativeInit");
	void (* Engine_Render) (void *env, void *obj) = (void *)so_symbol(&game_mod, "Java_org_cocos2dx_lib_Cocos2dxRenderer_nativeRender");
	void (* Engine_TouchesBegin) (void *env, void *obj, int id, float x, float y) = (void *)so_symbol(&game_mod, "Java_org_cocos2dx_lib_Cocos2dxRenderer_nativeTouchesBegin");
	void (* Engine_TouchesMove) (void *env, void *obj, int id, float x, float y) = (void *)so_symbol(&game_mod, "Java_org_cocos2dx_lib_Cocos2dxRenderer_nativeTouchesMove");
	void (* Engine_TouchesEnd) (void *env, void *obj, int id, float x, float y) = (void *)so_symbol(&game_mod, "Java_org_cocos2dx_lib_Cocos2dxRenderer_nativeTouchesEnd");
	void (* Engine_KeyDown) (void *env, void *obj, int key) = (void *)so_symbol(&game_mod, "Java_org_cocos2dx_lib_Cocos2dxRenderer_nativeKeyDown");

    if (!JNI_OnLoad) {
        // Fallback: JNI_OnLoad might be in libcocos2d.so
	...
	for (;;) {
        ...
        // Simple multitouch handling for standard Cocos2d-x
        for (int i = 0; i < SCE_TOUCH_MAX_REPORT; i++) {
            if (i < touch.reportNum) {
                ...
                if (lastX[i] == -1 || lastY[i] == -1) {
                    Engine_TouchesBegin(fake_env, NULL, touch.report[i].id, x, y);
                } else {
                    if (lastX[i] != x || lastY[i] != y)
                        Engine_TouchesMove(fake_env, NULL, touch.report[i].id, x, y);
                }
                lastX[i] = x;
                lastY[i] = y;
            } else {
                if (lastX[i] != -1 || lastY[i] != -1) {
                    Engine_TouchesEnd(fake_env, NULL, i, lastX[i], lastY[i]);
                    lastX[i] = -1;
                    lastY[i] = -1;
                }
            }
        ...
        // Map some keys. 4 = Back/Menu in Android keycodes usually
		if (((pad.buttons & SCE_CTRL_START) == SCE_CTRL_START) && !((oldpad & SCE_CTRL_START) == SCE_CTRL_START)) {
			Engine_KeyDown(fake_env, NULL, 4); // Android KEYCODE_BACK
		}
        
        // You can add more mappings here if the game supports controller via KeyEvents
		oldpad = pad.buttons;

		Engine_Render(fake_env, NULL);
...
int main(int argc, char *argv[]) {
    ...
    // Set path for POP Classic
	sprintf(data_path, "ux0:data/popclassic");
    ...
    // Removed old patch_game() call as offsets are invalid for new binary
