...
namespace SoLoud
{
	...
	result vita_homebrew_init(Soloud *aSoloud, unsigned int aFlags, unsigned int aSamplerate, unsigned int aBuffer, unsigned int aChannels)
	{
		...
		// INCREASED STACK SIZE: stb_vorbis uses a lot of stack to decode frames on the fly. 
		// 0x10000 (64KB) is not enough and causes a stack overflow (core dump). Increased to 0x20000 (128KB).
		data->tid = sceKernelCreateThread("soloud audio output", vita_thread, 0x10000100, 0x20000, 0, 0, NULL);
		if (data->tid >= 0) {
