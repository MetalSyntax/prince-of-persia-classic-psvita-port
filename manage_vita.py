...
def download_latest_debug_files():
    ...
    if descargar_dmp in ['s', 'y', 'si', 'yes']:
        ...
        try:
            ...
            if latest_dmp:
                local_dmp_name = f"PrinceOfPersiaClassic-{latest_dmp}"
    ...
    # 2. Obtener último log de ux0:/data/popclassic/logs
    ...
    try:
        ...
        print(f"[-] Error al buscar o descargar logs (asegúrate de que exista la ruta ux0:/data/popclassic/logs): {e}")
    finally:
        try:
            ftp.quit()
        except:
